...
(function($) {
   ...
   $.Scrollax();
	...
	if($(window).width() > 991) {
		$('nav .dropdown').hover(function(){
			...
			$this.find('.dropdown-menu').addClass('animated-fast fadeIn show');
		...
		$('nav .dropdown').click(function() {
			...
			if($(this).find('> div').attr('aria-expanded') === false) {
				$(this).addClass('show');
				$(this).find('> div').attr('aria-expanded', true);
  ...
  $('.popup-youtube, .popup-vimeo, .popup-gmaps').magnificPopup({
	...
	//categories in secondary navbar
	$.get('/product/getCategories', (data) => {
		for(var i=0; i<data.length; i++) {
			$('#navbar_main ul.navbar-nav').append('<li class="nav-item"><a class="nav-link" href="shop.html?'+data[i].id+'&'+data[i].value+'">'+data[i].value+'</a></li><hr class="secondary-nav-hr">')
		}
	});
	
	if($(window).width() < 992) {
		$('.secondary-nav-parent').removeClass('container');
	}
	else {
		$('.secondary-nav-parent').addClass('container');
	}
...
function menutox() {
	var menu = $('#menux');
	if(menu.hasClass('oi-menu')) {
		menu.removeClass('oi-menu');
		menu.addClass('oi-x');
	}
	else {
